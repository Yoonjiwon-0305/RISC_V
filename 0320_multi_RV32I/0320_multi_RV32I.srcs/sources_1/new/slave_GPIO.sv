...
module slave_GPIO (
    ...
    assign p_rdata = (p_sel) ? (
        ...
        (p_addr[11:0] == gpio_i_data_addr) ? {24'h0, gpio_i_data_reg[7:0]} :
        32'h0000_0000
    ) : 32'h0000_0000;
    ...
    always_ff @(posedge clk, posedge reset) begin
        if (reset) begin
            ...
            if (p_ready & p_write) begin
                case (p_addr[11:0])
                    gpio_ctl_addr:    gpio_ctl_reg    <= p_wdata[15:0];
                    gpio_o_data_addr: gpio_o_data_reg <= p_wdata[15:0];
                endcase
            end
        end
    end
